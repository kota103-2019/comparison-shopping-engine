...
class TokopediaSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        for kategori in kategori_collection.find():
            for url_tokopedia in kategori["tokopedia"]:
                #print(url_bukalapak)
                if url_tokopedia:
                    yield scrapy.Request(url=url_tokopedia, callback=self.parse, meta={
                "collection" : kota_collection,
    ...
    def parse_product(self, response):
        ...
        if new_product :
            ...
            product_object['seller'] = response.css("div.pdp-shop__info div.pdp-shop__info__name-wrapper span.shop-name::text").get()
            product_object['seller_url'] = response.css("div.pdp-shop__info div.pdp-shop__info__name-wrapper a::attr(href)").get()
            ...
            location_string = str(location_string).upper().replace("JABODETABEK", "JAKARTA PUSAT")
